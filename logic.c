...
SDL_Rect getAttackBox(Player *p) {
    ...
    r.y = (int)p->position.y - (p->rect.h / 2);
...
void updateCamera(GameState* gms){
    ...
    if (gms->p->position.x > SCREEN_WIDTH + *gms->camera_offset - CAMERA_GRACE) {
            *gms->camera_offset = gms->p->position.x + CAMERA_GRACE - SCREEN_WIDTH;
        }
        
        if (gms->p->position.x < *gms->camera_offset + CAMERA_GRACE) {
            *gms->camera_offset = gms->p->position.x - CAMERA_GRACE;
        }
        if (*gms->camera_offset < 0) {
            *gms->camera_offset = 0;
        }
        if (*gms->camera_offset > WORLD_MAX_X - SCREEN_WIDTH + gms->p->rect.w * gms->p->scale) {
            *gms->camera_offset = WORLD_MAX_X - SCREEN_WIDTH + gms->p->rect.w * gms->p->scale;
        }
...
void physics(Player* player, Entity* entities, GameSession* gameSession, GameState* gms, int*quit, int*flip, double *timer_s, int*debug, double* delta){

    userInputHandling(player, gameSession, gms, quit, flip, timer_s, debug);
...
void gameLoop(GameState *gms) {
    Player *player = gms->p;
    GameSession *gameSession = gms->gs;
    Entity *entities = gms->entites;
    ...
    double worldTime = *gms->worldTime; 
    double timer_s = 0;
    double delta;
    ...
    int camera_offset = *gms->camera_offset;
    
    double combo_delay = COMBO_DELAY_S;
